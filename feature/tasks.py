...
def broadcast_features_notification(info, organization_id: str):
    for user in User.objects.filter(role='Concierge'):
        logger.info(f'Send feature notification to: {user.email}')
        send_ws_message_to_group(
            room_id=CX_APP_ROOM,
            sender='Admin',
            receiver=user.email,
            logger=logger,
            event_type='FEATURES_UNLOCKED',
            payload=dict(
                type='update_feature_flags',
                sender=info.context.user.email,
                display_message=f'Features updated by {info.context.user}',
